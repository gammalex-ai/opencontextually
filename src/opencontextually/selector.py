...
# the project has ever shipped, so it matches almost any task's vocabulary
# and matches it repeatedly. That is the same repetition-of-description
# problem TEST_SIGNAL_DAMPING already handles for test files, and the same
# "written in feature vocabulary, but not the thing itself" problem behind
# EXAMPLE_PATH_PENALTY -- a changelog records what changed, it does not
# define how the code behaves now.
#
# Measured on the corpus: httpx's CHANGELOG.md took rank 8 of 18 on
# "redirect loses the authorization header", requests' HISTORY.md rank 6,
# and sqlfluff spent two of eighteen slots on release notes -- displacing
# `core/parser/markers.py` (`templated_position`) and flask's
# `wrappers.py` and `test_session_interface.py`. Damped rather than
# excluded: a changelog is occasionally the right answer ("when did this
# behaviour change?"), and it still ranks when nothing else matches.
#
# 0.5 rather than something heavier because both were measured against
# the corpus and produced identical results (18/19 found, 16/19 in the
# default view, docs share 33% -> 28%) -- but at 0.25 a changelog in a
# small repository falls below SCORE_THRESHOLD outright, which is a ban
# dressed up as a penalty. The weaker one buys the same thing honestly.
HISTORY_DOC_PENALTY = 0.5
_HISTORY_DOC_STEMS = {
    "changelog", "changes", "history", "news",
    "releasenotes", "release-notes", "release_notes",
    "whatsnew", "whats-new",
}
_HISTORY_DIR_SEGMENTS = {
    "releases", "release-notes", "releasenotes", "changelog", "changes",
}
...
def _is_history_document(path: str) -> bool:
    ...
    HISTORY_DOC_PENALTY, never to exclude the file outright.
    """
    name = PurePosixPath(path).name.lower()
    stem = name.rsplit(".", 1)[0] if "." in name else name
    if stem in _HISTORY_DOC_STEMS:
        return True
    dir_parts = path.lower().split("/")[:-1]
    return any(part in _HISTORY_DIR_SEGMENTS for part in dir_parts)
...
def _analyze(
    ...
    # Changelogs and release notes describe every feature ever shipped --
